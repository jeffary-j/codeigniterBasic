...
*/
	...
	*/
	function info()
	{
        ...
        if(empty($article_idx)|| !isset($post[0])){
            $this->load->helper('alert');
            alert('게시물이 없거나 정보가 잘못되었습니다.');
        }
	...
	*/
	function register()
	{
		...
		$header_data["css"] = array(
			base_url(ASSETS_CSS.'fileinput.css'),
			base_url(ASSETS_CSS.'plugins/dropzone/basic.css')
		);
		
		// footer에 추가되는 데이터
		$footer_data = array();
		$footer_data["js"] = array(
			base_url(ASSETS_JS.'form.post.js'),
			base_url(ASSETS_JS.'fileinput.js')
		);

        $this->load->view(VIEW_PAGE.'_front_header', $header_data);
		$this->load->view(VIEW_PAGE.'front_post_register', $data);
        $this->load->view(VIEW_PAGE.'_front_footer', $footer_data);
	...
	*/
	function input()
	{
		...
		if($idx){
			...
			$file_save_path = DOT.POST_UPFILE_PATH.DS.$user_idx.DS.$idx;
